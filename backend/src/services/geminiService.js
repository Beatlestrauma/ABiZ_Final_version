...
`;
  ...
  // Helper to parse JSON from AI responses (handles markdown fences)
  parseJSON(text) {
  ...
  async generateInsights(articles) {
    ...
    const prompt = `
You are a senior business intelligence analyst. Analyze these ${articles.length} current news articles and provide real insights.

ARTICLES:
${articleSummaries}

Analyze the articles and respond in STRICT JSON format (no markdown, no code fences):
{
  "marketSentiment": {
  ...
  "keyTrends": [
    {"trend": "<trend title>", "evidence": "<which articles support this>", "impact": "High|Medium|Low"}
  ],
  "riskFactors": [
    {"risk": "<risk description>", "severity": "High|Medium|Low", "affectedSectors": ["<sector>"]}
  ],
  "opportunities": [
    {"opportunity": "<opportunity description>", "timeframe": "Short|Medium|Long", "sector": "<sector>"}
  ],
  "sectorAnalysis": {
    "<sector name>": {"sentiment": "Positive|Negative|Neutral", "articleCount": <number>, "keyTheme": "<1 sentence>"}
  },
  "executiveSummary": "<2-3 sentence summary of the most important takeaways from today's news>"
}

Provide 3-5 items for keyTrends, riskFactors, and opportunities. Include 2-4 sectors in sectorAnalysis.
Focus on actionable business intelligence. Be specific to the actual articles provided.
`;
